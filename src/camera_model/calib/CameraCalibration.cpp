#include "lidar_localization/camera_model/calib/CameraCalibration.h"
...
#include "lidar_localization/camera_model/gpl/EigenQuaternionParameterization.h"
#include "lidar_localization/camera_model/gpl/EigenUtils.h"
#include "lidar_localization/camera_model/sparse_graph/Transform.h"
...
CameraCalibration::CameraCalibration(const Camera::ModelType modelType,
    ...
    m_camera = CameraFactory::instance()->generateCamera(modelType, cameraName,
                                                         imageSize);
}
...
void CameraCalibration::drawResults(std::vector<cv::Mat> &images) const {
    ...
    for (size_t i = 0; i < images.size(); ++i) {
        ...
        oss << "Reprojection error: avg = "
            << errorSum / m_imagePoints.at(i).size() << "   max = " << errorMax;
...
bool CameraCalibration::calibrateHelper(CameraPtr &camera,
    ...
    for (size_t i = 0; i < m_scenePoints.size(); ++i) {
        camera->estimateExtrinsics(m_scenePoints.at(i), m_imagePoints.at(i),
    ...
    if (m_verbose) {
        std::cout << "[" << camera->cameraName() << "] "
                  ...
                  << std::setprecision(3)
                  << camera->reprojectionError(m_scenePoints, m_imagePoints,
                                               rvecs, tvecs)
                  << " pixels" << std::endl;
    }
    ...
    if (m_verbose) {
        double err = camera->reprojectionError(m_scenePoints, m_imagePoints,
                                               rvecs, tvecs);
        std::cout << "[" << camera->cameraName() << "] "
...
void CameraCalibration::optimize(CameraPtr &camera, std::vector<cv::Mat> &rvecs,
                                 std::vector<cv::Mat> &tvecs) const {
