#include "lidar_localization/camera_model/camera_models/CostFunctionFactory.h"

#include "ceres/ceres.h"

#include "lidar_localization/camera_model/camera_models/CataCamera.h"
#include "lidar_localization/camera_model/camera_models/EquidistantCamera.h"
#include "lidar_localization/camera_model/camera_models/PinholeCamera.h"
#include "lidar_localization/camera_model/camera_models/ScaramuzzaCamera.h"
...
template <class CameraT> class ReprojectionError2 {
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
template <class CameraT> class ReprojectionError3 {
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    ...
    ReprojectionError3(const Eigen::Vector2d &observed_p,
                       const Eigen::Matrix2d &sqrtPrecisionMat)
        : m_observed_p(observed_p), m_sqrtPrecisionMat(sqrtPrecisionMat),
          m_optimize_cam_odo_z(true) {}
    ...
    ReprojectionError3(const std::vector<double> &intrinsic_params,
                       ...
                       const Eigen::Matrix2d &sqrtPrecisionMat)
        : m_intrinsic_params(intrinsic_params), m_observed_p(observed_p),
          m_sqrtPrecisionMat(sqrtPrecisionMat), m_optimize_cam_odo_z(true) {}
    ...
    ReprojectionError3(const std::vector<double> &intrinsic_params,
        ...
        : m_intrinsic_params(intrinsic_params), m_odo_pos(odo_pos),
          m_odo_att(odo_att), m_observed_p(observed_p),
    ...
    ReprojectionError3(const std::vector<double> &intrinsic_params,
                       ...
                       const Eigen::Vector3d &odo_att,
                       const Eigen::Vector2d &observed_p)
        : m_intrinsic_params(intrinsic_params), m_cam_odo_q(cam_odo_q),
          m_cam_odo_t(cam_odo_t), m_odo_pos(odo_pos), m_odo_att(odo_att),
          m_observed_p(observed_p), m_optimize_cam_odo_z(true) {}
...
// variables: camera intrinsics and camera extrinsics
template <class CameraT> class StereoReprojectionError {
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
template <class CameraT> class ComprehensionError {
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
ceres::CostFunction *CostFunctionFactory::generateCostFunction(
    ...
    case Camera::SCARAMUZZA:
        costFunction = new ceres::AutoDiffCostFunction<
            StereoReprojectionError<OCAMCamera>, 4,
            SCARAMUZZA_CAMERA_NUM_PARAMS, SCARAMUZZA_CAMERA_NUM_PARAMS, 4, 3, 4,
            3>(new StereoReprojectionError<OCAMCamera>(observed_P, observed_p_l,
                                                       observed_p_r));
        break;
    }
