...
OCAMCamera::Parameters::Parameters()
    : Camera::Parameters(SCARAMUZZA), m_C(0.0), m_D(0.0), m_E(0.0),
      m_center_x(0.0), m_center_y(0.0) {
...
void OCAMCamera::Parameters::writeToYamlFile(
    ...
    fs << "{"
       << "ac" << m_C << "ad" << m_D << "ae" << m_E << "cx" << m_center_x
       << "cy" << m_center_y << "}";
...
void OCAMCamera::estimateIntrinsics(
    ...
    // pseudo-inverse for polynomial parameters and all t3s
    {
        Eigen::JacobiSVD<Eigen::MatrixXd> svd(A_mat, Eigen::ComputeThinU |
                                                         Eigen::ComputeThinV);
