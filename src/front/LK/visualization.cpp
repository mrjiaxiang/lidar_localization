...
void registerPub(ros::NodeHandle &n) {
    pub_latest_odometry =
        n.advertise<nav_msgs::Odometry>("imu_propagate", 1000);
    pub_path = n.advertise<nav_msgs::Path>("path", 1000);
    pub_odometry = n.advertise<nav_msgs::Odometry>("odometry", 1000);
    pub_point_cloud = n.advertise<sensor_msgs::PointCloud>("point_cloud", 1000);
    pub_margin_cloud =
        n.advertise<sensor_msgs::PointCloud>("margin_cloud", 1000);
...
//     estimator.Vs[WINDOW_SIZE].transpose()); if (ESTIMATE_EXTRINSIC) {
//         cv::FileStorage fs(EX_CALIB_RESULT_PATH, cv::FileStorage::WRITE);
//         for (int i = 0; i < NUM_OF_CAM; i++) {
//             // ROS_DEBUG("calibration result for camera %d", i);
//             ROS_DEBUG_STREAM("extirnsic tic: " <<
//             estimator.tic[i].transpose()); ROS_DEBUG_STREAM("extrinsic ric: "
//                              <<
...
//         header.stamp.toSec(),
//                tmp_T.x(), tmp_T.y(), tmp_T.z(), tmp_Q.w(), tmp_Q.x(),
//                tmp_Q.y(), tmp_Q.z());
//     }
// }
...
//         if (STEREO) {
//             Vector3d P = estimator.Ps[i] + estimator.Rs[i] *
//             estimator.tic[1]; Quaterniond R = Quaterniond(estimator.Rs[i] *
//             estimator.ric[1]); cameraposevisual.add_pose(P, R);
//         }
...
//             Vector3d w_pts_i = estimator.Rs[imu_i] * (estimator.ric[0] *
//             pts_i +
//                                                       estimator.tic[0]) +
//                                estimator.Ps[imu_i];
...
//     transform.setOrigin(tf::Vector3(correct_t(0), correct_t(1),
//     correct_t(2))); q.setW(correct_q.w()); q.setX(correct_q.x());
//     q.setY(correct_q.y());
//     q.setZ(correct_q.z());
...
//         nav_msgs::Odometry odometry;
//         odometry.header.stamp = ros::Time(estimator.Headers[WINDOW_SIZE -
//         2]); odometry.header.frame_id = "world";
//         odometry.pose.pose.position.x = P.x();
//         odometry.pose.pose.position.y = P.y();
//         odometry.pose.pose.position.z = P.z();
//         odometry.pose.pose.orientation.x = R.x();
...
//                 p_2d.values.push_back(
//                     it_per_id.feature_per_frame[imu_j].point.x());
//                 p_2d.values.push_back(
//                     it_per_id.feature_per_frame[imu_j].point.y());
//                 p_2d.values.push_back(
//                     it_per_id.feature_per_frame[imu_j].uv.x());
//                 p_2d.values.push_back(
