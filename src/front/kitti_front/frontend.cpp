...
#include "lidar_localization/front/kitti_front/g2o_types.h"
...
bool Frontend::AddFrame(Frame::Ptr frame) {
    // cv::imwrite("/home/melody/test/left.png", frame->left_img_);
...
Frame::Ptr Frontend::NextFrame(cv::Mat &image_left, cv::Mat &image_right) {
    if (image_left.empty() || image_right.empty()) {
        LOG(INFO) << "cannot find images at index";
        return nullptr;
    }
    ...
    // LOG(INFO) << "current_image_index_ : " << current_image_index_;
    return new_frame;
}
...
bool Frontend::InsertKeyframe() {
    ...
    LOG(INFO) << "set frame " << current_frame_->id_ << " as keframe "
