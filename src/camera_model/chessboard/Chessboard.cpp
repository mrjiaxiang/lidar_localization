#include "lidar_localization/camera_model/chessboard/Chessboard.h"
#include <opencv2/calib3d/calib3d.hpp>
#include <opencv2/imgproc/imgproc.hpp>

#include "lidar_localization/camera_model/chessboard/ChessboardQuad.h"
#include "lidar_localization/camera_model/chessboard/Spline.h"
...
bool Chessboard::findChessboardCorners(const cv::Mat &image,
    ...
    if (useOpenCV) {
        ...
        return findChessboardCornersImproved(image, patternSize, corners,
                                             flags);
    }
}

bool Chessboard::findChessboardCornersImproved(
    ...
    /************************************************************************************\
        ...
        1) Martin Rufli - increased chance of correct corner matching
           ...
           Intelligent Robots and Systems (IROS 2008), Nice, France, September
    2008. 2) Lionel Heng - post-detection checks and better thresholding
    ...
    for (int k = 0; k < 6; ++k) {
        for (int dilations = minDilations; dilations <= maxDilations;
             ++dilations) {
            if (found) {
                break;
            }
            ...
            if (flags & cv::CALIB_CB_ADAPTIVE_THRESH) {
                int blockSize =
                    lround(prevSqrSize == 0 ? std::min(img.cols, img.rows) *
                                                  (k % 2 == 0 ? 0.2 : 0.1)
                                            : prevSqrSize * 2) |
                    1;
                ...
                cv::adaptiveThreshold(
                    img, thresh_img, 255, cv::ADAPTIVE_THRESH_MEAN_C,
                    cv::THRESH_BINARY, blockSize, (k / 2) * 5);
            } else {
                ...
                cv::threshold(img, thresh_img, thresh_level, 255,
                              cv::THRESH_BINARY);
            }
            ...
            cv::Mat kernel1 = cv::getStructuringElement(
                cv::MORPH_CROSS, cv::Size(3, 3), cv::Point(1, 1));
            ...
            // line around the image edge. Otherwise FindContours will miss
            // those clipped rectangle contours. The border color will be the
            // image mean, because otherwise we risk screwing up filters like
            // cvSmooth()
            ...
            // The connected quads will be organized in groups. The following
            // loop increases a "group_idx" identifier. The function
            // "findConnectedQuads assigns all connected quads a unique group
            // ID. If more quadrangles were assigned to a given group (i.e.
            // connected) than are expected by the input variable "patternSize",
            // the function "cleanFoundConnectedQuads" erases the surplus
...
// belong.
void Chessboard::cleanFoundConnectedQuads(
...
//===========================================================================
void Chessboard::findConnectedQuads(std::vector<ChessboardQuadPtr> &quads,
    ...
    while (!stack.empty()) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (neighbor.get() && neighbor->count > 0 &&
                neighbor->group_idx < 0) {
...
void Chessboard::labelQuadGroup(std::vector<ChessboardQuadPtr> &quadGroup,
                    ...
                    if (quad->neighbors[j]) {
                        ...
                        if (quadNeighbor->labeled) {
                            ...
                            quad->corners[(j + 1) % 4]->column =
                                ...
                                conCornerCW3->column;
                            quad->corners[(j + 2) % 4]->row = conCorner->row +
                                                              conCorner->row -
                                                              conCornerCW2->row;
    ...
    // there are an error would have been thrown at the beginning of
    ...
    for (int i = min_row; i <= max_row; ++i) {
        for (int j = min_column; j <= max_column; ++j) {
            ...
            for (int k = 0; k < (int)quadGroup.size(); ++k) {
                ...
                for (int l = 0; l < 4; ++l) {
                    if ((q->corners[l]->row == i) &&
                        (q->corners[l]->column == j)) {
                        if (flag) {
                ...
                for (int l = 0; l < 4; ++l) {
                    if ((q->corners[l]->row == i) &&
                        ...
                        if (number == 1) {
                            ...
                            cv::Point2f delta =
                                q->corners[l]->pt -
                                quadGroup[quadID]->corners[cornerID]->pt;
    ...
    // in a given dimension, is when the larger side has reached the target size
    // in the maximal direction, or if the larger side is larger than the
    // smaller target size and the smaller side equals the smaller target size
    ...
    if ((largerDimPattern + 1) == max_column - min_column) {
        ...
        // reached Therefore border column corners do not need a neighbor
        // anymore Go through all corners
        for (int k = 0; k < (int)quadGroup.size(); ++k) {
            ChessboardQuadPtr &q = quadGroup.at(k);
    ...
    if ((flagSmallerDim1 == true && flagSmallerDim2 == false)) {
        ...
        // smaller target pattern size is reached in row direction
...
// GIVE A GROUP IDX
//===========================================================================
void Chessboard::findQuadNeighbors(std::vector<ChessboardQuadPtr> &quads,
                                   int dilation) {
    ...
    for (size_t idx = 0; idx < quads.size(); ++idx) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (closestCornerIdx >= 0 && minDist < FLT_MAX) {
                ChessboardCornerPtr closestCorner =
                    closestQuad->corners[closestCornerIdx];
...
// own function
int Chessboard::augmentBestRun(std::vector<ChessboardQuadPtr> &candidateQuads,
    ...
    // conservative assumption that dilation was performed using the 3x3 CROSS
    // kernel, which coresponds to the 4-neighborhood.
    const float thresh_dilation = (2 * candidateDilation + 3) *
                                  (2 * existingDilation + 3) *
                                  2; // the "*2" is for the x and y component
    ...
    for (size_t idx = 0; idx < existingQuads.size(); ++idx) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (closestCornerIdx >= 0 && minDist < FLT_MAX) {
                ...
                newQuad->group_idx =
                    curQuad->group_idx;   // the same as the current quad
...
//===========================================================================
void Chessboard::generateQuads(std::vector<ChessboardQuadPtr> &quads,
                               cv::Mat &image, int flags, int dilation,
                               bool firstRun) {
    ...
    cv::findContours(image, contours, hierarchy, cv::RETR_CCOMP,
    ...
    for (size_t i = 0; i < contours.size(); ++i) {
        ...
        // "first run" is set to "true". This guarantees, that only "nice"
        // squares are detected. During PART 2, we allow the polygonal
        // approximation function below to approximate more freely, which can
        // result in recognized "squares" that are in reality multiple blurred
        // and sticked together squares.
        ...
        for (int approx_level = min_approx_level;
        ...
        if (approxContour.size() == 4 && cv::isContourConvex(approxContour)) {
            ...
            if (!(flags & cv::CALIB_CB_FILTER_QUADS) ||
                (d3 * 4 > d4 && d4 * 4 > d3 && d3 * d4 < area * 1.5 &&
                 area > minSize && d1 >= 0.15 * p && d2 >= 0.15 * p)) {
...
bool Chessboard::checkQuadGroup(std::vector<ChessboardQuadPtr> &quads,
    ...
    // exactly how the checkerboard is oriented. Else we need to prepare enough
    // "dummy" corners for the worst case.
    for (size_t i = 0; i < quads.size(); ++i) {
        ChessboardQuadPtr &q = quads.at(i);
        ...
        for (int j = 0; j < 4; ++j) {
            ...
            if (c->column == max_col && c->row != min_row &&
                c->row != max_row && !c->needsNeighbor) {
                flagColumn = true;
            }
            if (c->row == max_row && c->column != min_col &&
                c->column != max_col && !c->needsNeighbor) {
                flagRow = true;
            }
        }
    }
            ...
            for (int k = 0; k < (int)quads.size(); ++k) {
                ...
                for (int l = 0; l < 4; ++l) {
                    ...
                    if (c->row == i && c->column == j) {
                        ...
                        if (i == min_row || i == max_row || j == min_col ||
                            j == max_col) {
                            boardEdge = true;
                        }
                        ...
                        // more than two corners have the same row / column
                        // entries. Then some linking errors must have occured
                        // and we should not use the found pattern
                        if (iter > 2) {
                            return false;
                        }
    ...
    if ((int)corners.size() != patternSize.width * patternSize.height ||
        linkedBorderCorners <
            (patternSize.width * 2 + patternSize.height * 2 - 2) * 0.75f) {
        return false;
    }
    ...
    if ((p1 - p0).cross(p2 - p0) < 0.0f) {
        for (int i = 0; i < height; ++i) {
            for (int j = 0; j < width / 2; ++j) {
                std::swap(corners.at(i * width + j),
                          corners.at(i * width + width - j - 1));
            }
        }
    }
...
void Chessboard::getQuadrangleHypotheses(
...
bool less_pred(const std::pair<float, int> &p1,
               const std::pair<float, int> &p2) {
    return p1.first < p2.first;
}

void countClasses(const std::vector<std::pair<float, int>> &pairs, size_t idx1,
                  size_t idx2, std::vector<int> &counts) {
    counts.assign(2, 0);
...
bool Chessboard::checkChessboard(const cv::Mat &image,
    ...
    bool result = false;
    for (float threshLevel = blackLevel; threshLevel < whiteLevel && !result;
         threshLevel += 20.0f) {
        cv::threshold(white, thresh, threshLevel + blackWhiteGap, 255,
                      cv::THRESH_BINARY);
        ...
        const size_t min_quads_count =
            patternSize.width * patternSize.height / 2;
        ...
        for (size_t i = 0; i < quads.size(); ++i) {
            ...
            if (j + 1 > min_quads_count + i) {
                ...
                const int blackCount =
                    lroundf(ceilf(patternSize.width / 2.0f) *
                            ceilf(patternSize.height / 2.0f));
                const int whiteCount =
                    lroundf(floorf(patternSize.width / 2.0f) *
                            floorf(patternSize.height / 2.0f));
...
bool Chessboard::checkBoardMonotony(std::vector<ChessboardCornerPtr> &corners,
    ...
    for (int i = 0; i < patternSize.height; ++i) {
        ...
        for (int j = 1; j < patternSize.width - 1; ++j) {
            ...
            if (i > 0) {
                cv::Point2f &neighbor =
                    corners.at((i - 1) * patternSize.width + j)->pt;
            ...
            {
                cv::Point2f &neighbor =
                    corners.at(i * patternSize.width + j - 1)->pt;
                thresh = fminf(thresh, cv::norm(neighbor - p_j));
            }
            // right-neighbor
            {
            ...
            if (fminf(fabsf(splineXY(p_j.x) - p_j.y),
                      fabsf(splineYX(p_j.y) - p_j.x)) > thresh) {
                return false;
            }
        }
    }
    ...
    for (int j = 0; j < patternSize.width; ++j) {
        ...
        for (int i = 1; i < patternSize.height - 1; ++i) {
            ...
            if (fminf(fabsf(splineXY(p_i.x) - p_i.y),
                      fabsf(splineYX(p_i.y) - p_i.x)) > thresh) {
                return false;
            }
        }
    }
...
bool Chessboard::matchCorners(ChessboardQuadPtr &quad1, int corner1,
    ...
    // sides 1
    float x1 = (quad1->corners[corner1]->pt.x +
                quad1->corners[(corner1 + 1) % 4]->pt.x) /
               2;
    float y1 = (quad1->corners[corner1]->pt.y +
                quad1->corners[(corner1 + 1) % 4]->pt.y) /
               2;
    float x2 = (quad1->corners[(corner1 + 2) % 4]->pt.x +
                quad1->corners[(corner1 + 3) % 4]->pt.x) /
               2;
    float y2 = (quad1->corners[(corner1 + 2) % 4]->pt.y +
                quad1->corners[(corner1 + 3) % 4]->pt.y) /
               2;
    // compute midpoints of "parallel" quad sides 2
    float x3 = (quad1->corners[corner1]->pt.x +
                quad1->corners[(corner1 + 3) % 4]->pt.x) /
               2;
    float y3 = (quad1->corners[corner1]->pt.y +
                quad1->corners[(corner1 + 3) % 4]->pt.y) /
               2;
    float x4 = (quad1->corners[(corner1 + 1) % 4]->pt.x +
                quad1->corners[(corner1 + 2) % 4]->pt.x) /
               2;
    float y4 = (quad1->corners[(corner1 + 1) % 4]->pt.y +
                quad1->corners[(corner1 + 2) % 4]->pt.y) /
               2;
    ...
    // quad sides 1
    float u1 = (quad2->corners[corner2]->pt.x +
                quad2->corners[(corner2 + 1) % 4]->pt.x) /
               2;
    float v1 = (quad2->corners[corner2]->pt.y +
                quad2->corners[(corner2 + 1) % 4]->pt.y) /
               2;
    float u2 = (quad2->corners[(corner2 + 2) % 4]->pt.x +
                quad2->corners[(corner2 + 3) % 4]->pt.x) /
               2;
    float v2 = (quad2->corners[(corner2 + 2) % 4]->pt.y +
                quad2->corners[(corner2 + 3) % 4]->pt.y) /
               2;
    // compute midpoints of "parallel" quad sides 2
    float u3 = (quad2->corners[corner2]->pt.x +
                quad2->corners[(corner2 + 3) % 4]->pt.x) /
               2;
    float v3 = (quad2->corners[corner2]->pt.y +
                quad2->corners[(corner2 + 3) % 4]->pt.y) /
               2;
    float u4 = (quad2->corners[(corner2 + 1) % 4]->pt.x +
                quad2->corners[(corner2 + 2) % 4]->pt.x) /
               2;
    float v4 = (quad2->corners[(corner2 + 1) % 4]->pt.y +
                quad2->corners[(corner2 + 2) % 4]->pt.y) /
               2;
